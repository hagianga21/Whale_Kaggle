...
img_train = train_df['Image'].values
label_train = y

img_val = train_df['Image'].values[perm][:1000]
label_val = y[perm][:1000]
print('Hello')
print(img_val.shape)
...
train_dataset= WhaleDataset2(datafolder='../data/train/', datatype='train', filenames = img_train, labels = label_train, transform=datatrain_transforms)
val_dataset= WhaleDataset2(datafolder='../data/train/', datatype='train', filenames = img_val, labels = label_val, transform=dataval_transforms)
...
########## 
print('Load model')
saved_model_fn = 'resnetper' + '-%s' % (args.depth) + '_' + strftime('%m%d') + '_' + '%s' %(args.batch_size) + '%s' %(args.frozen_until)
old_model = './checkpoint/' + 'resnetper' + '-%s' % (args.depth) + '_' + args.model_path + '.t7'
